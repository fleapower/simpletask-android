...
class Preferences : ThemedPreferenceActivity(), SharedPreferences.OnSharedPreferenceChangeListener {
        ...
        m_broadcastReceiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context, receivedIntent: Intent) {
                if (receivedIntent.action == Constants.BROADCAST_THEME_CHANGED) {
                    Log.i(
                        TAG, "Reloading preference screen with fragment ${
    ...
    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences?, key: String?) {
        when (key) {
            getString(R.string.calendar_sync_thresholds), getString(R.string.calendar_sync_dues) -> requestCalendarPermission()
            getString(R.string.theme_pref_key) -> {
                onContentChanged()
                val broadcastIntent = Intent(Constants.BROADCAST_THEME_CHANGED)
                intent.putExtra(
                    EXTRA_SHOW_FRAGMENT, AppearancePrefFragment::class.java.name
                )
            ...
            getString(R.string.custom_font_size), getString(R.string.font_size) -> {
    ...
    private fun requestCalendarPermission() {
        if (TodoApplication.config.isSyncDues || TodoApplication.config.isSyncThresholds) {
            val writePermissionCheck = ContextCompat.checkSelfPermission(
                app, Manifest.permission.WRITE_CALENDAR
            )
            if (writePermissionCheck == PackageManager.PERMISSION_DENIED) {
                ActivityCompat.requestPermissions(
                    this, arrayOf(Manifest.permission.WRITE_CALENDAR), 0
                )
            }
            ...
            if (readPermissionCheck == PackageManager.PERMISSION_DENIED) {
                ActivityCompat.requestPermissions(
                    this, arrayOf(Manifest.permission.READ_CALENDAR), 0
                )
            }
        }
    }
...
fun Preference.valueInSummary(any: Any? = null) {
    this.summary = this.summary.replaceFirst(
        Regex("%s"), any?.toString() ?: this.sharedPreferences.getString(this.key, "") ?: ""
    )
}