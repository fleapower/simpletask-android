...
class Simpletask : ThemedNoActionBarActivity() {
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        taskAdapter = TaskAdapter(completeAction = {
            completeTasks(it)
            ...
            TodoApplication.todoList.notifyTasklistChanged(
                TodoApplication.config.todoFile, save = false, refreshMainUI = true
            )
        }, unCompleteAction = {
            uncompleteTasks(it)
            // Update the tri state checkbox
            handleMode(mapOf(Mode.SELECTION to { invalidateOptionsMenu() }))
            TodoApplication.todoList.notifyTasklistChanged(
                TodoApplication.config.todoFile, true
            )
        }, onClickAction = {
                ...
                for (i in links.indices) {
                    when (actions[i]) {
                        Action.SMS -> titles.add(
                            i, getString(R.string.action_pop_up_sms) + links[i]
                        )

                        Action.PHONE -> titles.add(
                            i, getString(R.string.action_pop_up_call) + links[i]
                        )
        ...
        val broadcastReceiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context, receivedIntent: Intent) {
                ...
                } else if (receivedIntent.action == Constants.BROADCAST_THEME_CHANGED || receivedIntent.action == Constants.BROADCAST_DATEBAR_SIZE_CHANGED) {
                    recreate()
    ...
    override fun onBackPressed() {
        handleMode(mapOf(Mode.SAVED_FILTER_DRAWER to {
    ...
    private fun selectAllTasks() {
        val selectedTasks =
            taskAdapter.visibleLines.filterNot(VisibleLine::header).map { it.task!! }
    ...
    override fun onCreateOptionsMenu(menu: Menu): Boolean {
        ...
        handleMode(
            mapOf(Mode.SAVED_FILTER_DRAWER to {
    ...
    private fun completeTasks(tasks: List<Task>) {
        TodoApplication.todoList.complete(
            tasks, TodoApplication.config.hasKeepPrio, TodoApplication.config.hasAppendAtEnd
        )
                ...
                if (input == "pick") {
                    ...
                    val dialog = DatePickerDialog(this@Simpletask, { _, year, month, day ->
                        ...
                        TodoApplication.todoList.defer(
                            date.format(Constants.DATE_FORMAT), tasks, dateType
                        )
                        TodoApplication.todoList.notifyTasklistChanged(
                            TodoApplication.config.todoFile, true
                        )
    ...
    private fun deleteTasks(tasks: List<Task>) {
        ...
        val title =
            getString(R.string.delete_task_title).replaceFirst(Regex("%s"), numTasks.toString())
    ...
    private fun archiveTasks(showDialog: Boolean = true) {
        ...
        if (selection.isNotEmpty()) {
            ...
            tasksToArchive.addAll(taskAdapter.visibleLines.asSequence().filterNot { it.header }
                .map { (it as TaskLine).task }.filter { it.isCompleted() })
        }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        when (item.itemId) {
            androidId.home -> {
                handleMode(
                    mapOf(Mode.SAVED_FILTER_DRAWER to { closeDrawer(SAVED_FILTER_DRAWER) },
                ...
                FileStoreActionQueue.add("Exporting filters") {
                    try {
                        ...
                        QueryStore.exportFilters(
                            File(
                                TodoApplication.config.todoFile.parentFile, filename
                            )
                        )
            ...
            R.id.menu_export_filter_import -> {
                FileStoreActionQueue.add("Importing filters") {
                    val filename = if (FileStore.isEncrypted) {
                        "saved_filters.txt.jenc"
                    } else {
                        "saved_filters.txt"
                    }
    ...
    private fun createCalendarAppointment(checkedTasks: List<Task>) {
        ...
        intent.putExtra(
            CalendarContract.EXTRA_EVENT_BEGIN_TIME, calDate.timeInMillis
        )
        intent.putExtra(
            CalendarContract.EXTRA_EVENT_END_TIME, calDate.timeInMillis + 60 * 60 * 1000
        )
    ...
    private fun startAddTaskActivity() {
        ...
        TodoApplication.todoList.editTasks(
            this, TodoApplication.todoList.selectedTasks, TodoApplication.config.mainQuery.prefill
        )
    }

    private fun startPreferencesActivity() {
        val settingsActivity = Intent(
            baseContext, Preferences::class.java
        )
    ...
    private fun pinNotification(checkedTasks: List<Task>) {
        ...
        for (task in checkedTasks) {
            ...
            var builder = NotificationCompat.Builder(this, "pin-notifications")
                .setSmallIcon(R.drawable.ic_done_white_24dp).setContentTitle(task.text)
                .setPriority(NotificationCompat.PRIORITY_DEFAULT).setContentIntent(editTaskIntent)
    ...
    private inner class UiHandler() {
        fun forEvent(event: Event) {
            ...
            runOnUiThread {
                when (event) {
                    ...
                    Event.QUICK_FILTER_ITEM_CLICK, Event.CLEAR_FILTER -> {
                        updateTaskList(TodoApplication.config.mainQuery) {
                            updateFilterBar()
                            updateQuickFilterDrawer()
                        }
                    ...
                    Event.SAVED_FILTER_ADDED, Event.SAVED_FILTER_RENAMED, Event.SAVED_FILTER_UPDATED, Event.SAVED_FILTER_DELETED, Event.SAVED_FILTERS_IMPORTED -> {
                        ...
                        popupMenu.setOnMenuItemClickListener { item ->
                            ...
                            when (menuId) {
                                ...
                                R.id.menu_saved_filter_update -> updateSavedQuery(
                                    query.second, TodoApplication.config.mainQuery
                                )
        ...
        private fun updateQuickFilterDrawer() {
            ...
            TodoApplication.config.mainQuery.contexts.asSequence()
                .map { drawerAdapter.getIndexOf("@$it") }.filter { it != -1 }
            ...
            binding.filterDrawer.setItemChecked(
                drawerAdapter.contextHeaderPosition, TodoApplication.config.mainQuery.contextsNot
            )
            binding.filterDrawer.setItemChecked(
                drawerAdapter.projectsHeaderPosition, TodoApplication.config.mainQuery.projectsNot
            )
...
enum class Event {
    TASK_LIST_CHANGED, FILTER_CHANGED, QUICK_FILTER_ITEM_CLICK, SAVED_FILTER_ITEM_CLICK, SAVED_FILTER_ADDED, SAVED_FILTER_UPDATED, SAVED_FILTER_DELETED, SAVED_FILTERS_IMPORTED, SAVED_FILTER_RENAMED, RESUME, FONT_SIZE_CHANGED, UPDATE_PENDING_CHANGES, CLEAR_FILTER, CLEAR_SELECTION
}
